...
async def _(event):
    if ispay2[0] == "yes":
        ...
        choice = str(msg[1])
        trys = 0
        await event.edit(f"𝐈 𝐰𝐢𝐥𝐥 𝐜𝐡𝐞𝐜𝐤 `{choice}` 𝐟𝐫𝐨𝐦 𝐭𝐡𝐞 𝐮𝐬𝐞𝐫  𝐨𝐧 `{ch}` 𝐚 𝐧𝐮𝐦𝐛𝐞𝐫 {msg[0]} 𝐨𝐟 𝐚𝐭𝐭𝐞𝐦𝐩𝐭𝐬 .")

        @sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
                    await event.edit(f"𝐭𝐡𝐞 𝐩𝐫𝐨𝐠𝐫𝐞𝐬𝐬 𝐨𝐟 𝐭𝐡𝐞 𝐞𝐱𝐚𝐦𝐢𝐧𝐚𝐭𝐢𝐨𝐧 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 `({trys})` 𝐚 𝐧𝐮𝐦𝐛𝐞𝐫 𝐨𝐟 𝐚𝐭𝐭𝐞𝐦𝐩𝐭𝐬")
                elif "off" in isclaim:
                    await event.edit("𝐓𝐡𝐞 𝐡𝐮𝐧𝐭𝐬 𝐢𝐬 𝐧𝐨𝐭 𝐰𝐨𝐫𝐤𝐢𝐧𝐠")
                else:
...
𝐛𝐲 ➯ @xxizx
    ...
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        ...
                        await sedthon.send_message(event.chat.id, "𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞 𝐮𝐬𝐞𝐫𝐧𝐚𝐦𝐞")
            else:
                pass
            trys += 1
        ...
        isclaim.append("off")
        trys = ""
        await event.client.send_message(event.chat_id, "𝐡𝐮𝐧𝐭𝐢𝐧𝐠 𝐢𝐬 𝐨𝐯𝐞𝐫")
    else:
        await event.edit("يجب الدفع لاستعمال هذا الامر !")
